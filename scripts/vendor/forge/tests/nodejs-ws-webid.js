...
var authenticateWebId = function (c, state) {
    ...
    sys.log(
        'Server UserVerify certificate w/CN: \"' +
...
// creates UserRegister (private key + certificate)
var createCredentials = function (cn, credentials) {
    ...
    // save UserRegister
    UserRegister.key = forge.pki.privateKeyToPem(keys.privateKey);
    UserRegister.cert = forge.pki.certificateToPem(cert);
...
var readPrivateKey = function (filename) {
    UserRegister.key = fs.readFileSync(filename);
    // try to parse from PEM as test
    forge.pki.privateKeyFromPem(UserRegister.key);
};

// read certificate from file
var readCertificate = function (filename) {
    UserRegister.cert = fs.readFileSync(filename);
    // try to parse from PEM as test
    forge.pki.certificateFromPem(UserRegister.cert);
};
...
// create UserRegister for server
if (UserRegister.key === null || UserRegister.cert === null) {
    createCredentials('server', UserRegister);
}
